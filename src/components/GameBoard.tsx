...
export const GameBoard: React.FC<GameBoardProps> = ({ initialRoom, playerProfile, onlineSession, onExit }) => {
      ...
      connectOnlineSocket(onlineSession.sessionToken, {
        onGameState: (state) => {
          ...
          const myTurn =
            ...
            state.players[state.currentTurnIndex]?.id === onlineSession.memberId;
  ...
  const handleDrawCard = (source: 'draw' | 'discard') => {
    if (!isMyTurn || turnState !== 'drawing') return;
  ...
  const executeDiscard = (card: Card, skipPlayerId: string | null) => {
    ...
    let playersAfterDiscard: Player[] = [];
    ...
    setRoom(prev => {
      ...
      playersAfterDiscard = updatedPlayers;
    ...
    const endResult = evaluateRoundEnd(playersAfterDiscard, room.laidDownPhases, {
  ...
  const advanceTurn = () => {
    setRoom(prev => {
      ...
      // Keep checking if the next player is skipped, we unskip them and jump to next!
      let skipSafetyCount = 0;
      while (prev.players[nextIndex].isSkipped && skipSafetyCount < prev.players.length) {
        const skippedPlayerName = prev.players[nextIndex].name;
        // Unskip
        prev.players[nextIndex].isSkipped = false;
        addLog(`🚫 ${prev.players[nextIndex].avatar} ${skippedPlayerName} foi pulado!`, 'warning');
